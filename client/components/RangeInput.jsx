...
const RangeInput = ({rangeName, id, value, onChange, maxValue }) => {
  return (
    <>
        ...
        <div className='flex space-x-2 py-2 items-center'>
            <input type="range" id={id} name={id} value={value} max={maxValue} onChange={(e) => onChange(e.target.value)} className='border p-2 border-gray-300 rounded w-full'/>
        </div>
    </>
  )
}
